...
Desktop-Tool (PowerShell + WPF) zum Sichern, Verwalten und Wiederherstellen lokaler
Diablo-II-Resurrected-Charaktere — mit Labels, Tags, Notizen und Wiederherstellung
unter einem anderen Charakternamen.

Bedienungsanleitungen für Endanwender: [ANLEITUNG.md](ANLEITUNG.md) (deutsch),
[INSTRUCTIONS.md](INSTRUCTIONS.md) (englisch). Dieses README richtet sich an den,
der den Code pflegt.
...
**Parken** ist bisher nur in der Sandbox erprobt, nie gegen den echten Spielstand-Ordner.
...
| `README.md` | diese Datei, nur für die Pflege |
...
| `Migrate-Backups.ps1` | einmalige Umstellung ZIP → Ordner, bereits gelaufen |
...
`Migrate-Backups.ps1` hat den Altbestand einmalig umgestellt — 65 von 65 am 03.08.2026,
...
1. **Parken noch nie echt ausgeführt.** Nur Sandbox. Erster echter Test: D2R beenden →
   alles sichern → einen unwichtigen Charakter parken → D2R starten und prüfen, ob er
   aus der Auswahl verschwunden ist → zurückholen → prüfen, ob er wieder da ist.
2. **Altbestände nicht importiert.** Die vorhandenen Ordner `Backup`, `BAK20250510`, `Temp`
...
3. **„Alles sichern" erfasst geparkte Charaktere nicht.** Siehe *Ablage*. Bewusst so
   gelassen, weil das Ändern auch `Restore-Snapshot` betrifft — aber es ist eine Falle,
   falls „komplett" wörtlich genommen wird.
4. **„Geparkte ausblenden" überlebt den Neustart nicht.** Bewusst so entschieden
   (04.08.2026), weil unklar war, ob das Merken überhaupt gewünscht ist. Wer viele
   Charaktere dauerhaft geparkt hat, setzt den Haken sonst nach jedem Start neu.
   Umsetzung wäre klein: Wert wie `SortChars` in `config.json` ablegen, beim Aufbau
   des Fensters setzen. Die vorhandene Checkbox „Auto-Sicherungen ausblenden" hat
   dasselbe Verhalten — beide zusammen wären konsequenter als nur eine.
5. **Kosmetik.** Die Griffpunkte und der Überlaufpfeil der Werkzeugleiste sind
