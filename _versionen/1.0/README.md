...
`Test-Sandbox.ps1` deckt 48 Prüfungen ab (Header-Parsing, Sichern, Wiederherstellen,
Umbenennen, Ordner-Ablage, `_INFO.txt`, Stash-Verhalten, Sicherheitskopie, Löschen,
Index-Neuladen) und läuft grün unter Windows PowerShell 5.1 **und** PowerShell 7.4.
...
| `Test-Sandbox.ps1` | 48 Prüfungen gegen eine Wegwerf-Sandbox |
...
Die Ordnernamen `Charaktere`, `Kompletter Ordner` und `SharedStash` sind **fest und werden nicht
übersetzt** — sonst entstünden beim Sprachwechsel zwei Bäume.
...
1. **Altbestände nicht importiert.** Die vorhandenen Ordner `Backup`, `BAK20250510`, `Temp`
   und die 178 `.bak`-Dateien im Spielstand-Ordner ignoriert die App bisher.
2. **Kosmetik.** Die Griffpunkte und der Überlaufpfeil der Werkzeugleiste sind
